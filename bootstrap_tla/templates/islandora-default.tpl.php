...
<fieldset class="collapsible" style="display: block; clear:both">
    <legend><a href="#fs" class="fieldset-legend" data-toggle="collapse"><?php print t('Downloadable metadata for this object'); ?></a></legend>
    <div id="fs" class="fieldset-wrapper">
        <table>
            <tr>
                <!-- <th><?php print t('ID'); ?></th> -->
                <th><?php print t('Download link&nbsp'); ?></th>
                <th><?php print t('Size'); ?></th>
                <!--<th><?php print t('Mimetype'); ?></th>-->
                <th><?php print t('Created'); ?></th>
            </tr>
            <?php foreach($datastreams as $key => $value): ?>
                <?php if(isset($value['id']) AND ($value['id'] == 'CMD' OR $value['id'] == 'DC')):  ?>
                    <tr>
                        <!--<td><?php if(isset($value['id'])): ?><?php print $value['id']; ?><?php endif; ?></td>-->
                        <td><?php if(isset($value['label_link'])): ?><?php $name = $islandora_object->label . ' ' . $value['id'];
                                $link = l($name, $value['download_url'], array('attributes'=>array('download'=> $value['id'] . '.xml')));
                                print $link;?><?php endif; ?></td>
